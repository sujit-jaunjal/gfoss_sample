...
b.pageY;d.css({left:k>0&&a.positionBy!="bottomTop"?k:o+n/2>K?K/2-n/2:Math.max(o-n/2,0),zIndex:e.data("cluetip").zIndex});s.css({zIndex:e.data("cluetip").zIndex+1})}ba=c(window).height();if(j){if(typeof j=="function")j=j.call(h);g.html(j);L(f)}else if(G){b=G.length;g.html(b?G[0]:"");if(b>1)for(var i=1;i<b;i++)g.append('<div class="split-body">'+G[i]+"</div>");L(f)}else if(!a.local&&m.indexOf("#")!==0)if(/\.(jpe?g|tiff?|gif|png)(?:\?.*)?$/i.test(m)){g.html('<img src="../html/js/'+m+'" alt="'+u+'" />');L(f)}else{var l=
